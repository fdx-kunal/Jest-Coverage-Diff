...
export class DiffChecker {
    ...
    getCoverageDetails(diffOnly: boolean, currentDirectory: string): string[] {
        ...
        for (const key of keys) {
            if (this.compareCoverageValues(this.diffCoverageReport[key]) !== 0) {
                returnStrings.push(
                    this.createDiffLine(key.replace(currentDirectory, ""), this.diffCoverageReport[key]),
                );
            } else {
                if (!diffOnly) {
                    returnStrings.push(
                        `${key.replace(currentDirectory, "")} | ${this.diffCoverageReport[key].statements.newPct} | ${
    ...
    checkIfTestCoverageFallsBelowDelta(delta: number, totalDelta: number | null): boolean {
    ...
    private createDiffLine(name: string, diffFileCoverageData: DiffFileCoverageData): string {
        ...
        return ` ${statusIcon} | ${name} | ${diffFileCoverageData.statements.newPct} **(${this.getPercentageDiff(
            diffFileCoverageData.statements,
        )})** | ${diffFileCoverageData.branches.newPct} **(${this.getPercentageDiff(
            diffFileCoverageData.branches,
        )})** | ${diffFileCoverageData.functions.newPct} **(${this.getPercentageDiff(
            diffFileCoverageData.functions,
        )})** | ${diffFileCoverageData.lines.newPct} **(${this.getPercentageDiff(diffFileCoverageData.lines)})**`;
    }
    ...
    private getStatusIcon(diffFileCoverageData: DiffFileCoverageData): ":green_circle:" | ":red_circle:" {
        let overallDiff = 0;
